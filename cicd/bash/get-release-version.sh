...
#   branch_name: the formatted name of the branch being built. Will be "master" if not a feature branch.
...
packageVersion=$(node -pe "require('./package.json').version" | sed 's/\.0$//')
...
if [[ $branch_name == 'master' ]]
    then
        ...
        readonly releaseVersion="v$packageVersion.$versionPatch-$branch_name"
fi
